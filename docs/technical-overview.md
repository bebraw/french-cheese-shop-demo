...
room-scoped rather than account-based: one device can claim shared-query,
world-context, and challenge-management rights for the room, and the UI exposes
separate lecturer and audience share links so participants do not receive that
control by default.
